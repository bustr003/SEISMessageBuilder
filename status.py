...
def make_eligible():
    ...
    # CREATE THE FRAMES
    request_frame = w.f_make_eligible # !! FRAME
...
"""
Change a student's status from Pending to Ineligible
"""
def make_DNQ():
    bg_color = w.red # !! BG COLOR

    # CREATE THE TITLE
    reqType = "Status: Make DNQ" # !! REQUEST TYPE

    # CREATE THE FRAMES
    request_frame = w.f_make_DNQ # !! FRAME
    close_frame = w.Frame(request_frame)
    input_frame = w.Frame(request_frame)
    note_frame = w.Frame(request_frame)
